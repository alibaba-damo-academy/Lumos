...
```
# Create a new conda environment named 'lumos-1-public' with Python 3.10
conda create -n lumos-1-public python=3.10 -y
# Activate the 'lumos-1-public' environment
...
Lumos-1 implementation heavily relies on xllmx and requires xllmx to be installed as a python package (**so that `import xllmx` can be used anywhere in your machine, without the restriction of working directory**).
The installation process is as follows:
```bash
# bash
# go to the root path of the project
cd Lumos-Project/lumos-1
# install as package
pip install -e .
```

> **Note:** The following step is mentioned by the Lumina-mGPT project, which is not used in Lumos-1.
> Feel free to explore its usage if you find it super useful.
