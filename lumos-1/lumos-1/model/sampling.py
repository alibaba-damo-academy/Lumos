...
def mask_by_random_topk(mask_len, probs, temperature=1.0, generator=None):
    # The function computes a "confidence" score for each token based on the log-probabilities (log(probs)) of the tokens.
    # The gumbel_noise function adds noise for randomness, scaled by temperature. This technique, called Gumbel-Softmax, helps simulate sampling from a categorical distribution, making the masking more diverse.
    confidence = log(probs) + temperature * gumbel_noise(probs, generator=generator)
    # The tokens are sorted based on their confidence scores.
    sorted_confidence = torch.sort(confidence, dim=-1).values
    # cut_off determines the confidence value below which tokens will be masked. It is chosen such that mask_len tokens will be masked, ensuring diversity.
