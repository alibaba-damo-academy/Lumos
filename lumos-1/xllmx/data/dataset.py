...
class FinetuneConversationDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # meta_idx is the index of the meta data collection (group of items) that the item belongs to.
        # idx_in_meta is the index of the item within that specific metadata collection.
        # breakpoint()

        try:
            return self.get_item_func(meta_idx, idx_in_meta)
            # get_item_func fetches the corresponding data item (from self.annotations_collection) 
            # and processes it using self.item_processor.process_item().
