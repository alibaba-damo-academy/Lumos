...
def replace_nth_str(string, sub, wanted, n):
    """
    Replaces the n-th occurrence of a substring in a string with a new substring.

    Parameters:
    string (str): The original string where replacements will be performed.
    sub (str): The substring to search for and replace.
    wanted (str): The string that will replace the n-th occurrence of 'sub'.
    n (int): The occurrence number of 'sub' to replace (1-based index).

    Returns:
    str: A new string with the n-th occurrence of 'sub' replaced by 'wanted'.
          If the n-th occurrence is not found, the original string is returned unchanged.
    """

    # Use re.finditer but only iterate until we find the nth occurrence
    match_iter = re.finditer(sub, string)
    for i, match in enumerate(match_iter, start=1):
        if i == n:
            where = match.start()
            return string[:where] + wanted + string[where + len(sub):]
    
    # If we reach here, n-th occurrence was not found
    return string  # Return original string if n-th occurrence is not found