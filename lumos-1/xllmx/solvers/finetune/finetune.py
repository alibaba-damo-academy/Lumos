...
class FinetuneSolverBase(ABC):
    ...
    def __init__(self, args):
        ...
        # print(f"self.tokenizer.tokenizer {len(self.tokenizer.tokenizer)}")
        # breakpoint()
    ...
    def build_model(self) -> (nn.Module, Tokenizer):
        ...
        if self.args.checkpointing:
            ...
            self.logger.info("apply gradient checkpointing")
    ...
    def resume(self, resume_path: str):
        ...
        _load_optimizer()
        # # Release any unreferenced cached memory # NOTE: This is a test.
        # torch.cuda.empty_cache()
    ...
    # NOTE: This is a test for resolving the failure of resume training function. This can be deleted if it is of no use.
    # def resume(self, resume_path: str):
    #     """
    #     Note: We assume that the model constructor or _model_func is NOT already loading
    #     the model to GPU. If it is, you may need to modify that to load on CPU first.
    #     """

    #     # -----------------------------
    #     # 1) Load the optimizer state on CPU
    #     # -----------------------------
    #     def _load_optimizer():
    #         opt_state_world_size = len(
    #             [x for x in os.listdir(resume_path) if x.startswith("optimizer.") and x.endswith(".pth")]
    ...
    #         # Load optimizer state_dict onto CPU
    #         optimizer_ckpt_path = os.path.join(
    #             resume_path, f"optimizer.{dist.get_rank():05d}-of-{dist.get_world_size():05d}.pth"
    #         )
    #         optimizer_ckpt = torch.load(optimizer_ckpt_path, map_location="cpu")
    #         self.optimizer.load_state_dict(optimizer_ckpt)

    #         # Make sure the LR, WD, etc. are overwritten from command-line args
    #         for param_group in self.optimizer.param_groups:
    #             param_group["lr"] = self.args.lr
    #             param_group["weight_decay"] = self.args.wd

    #     # Actually load the optimizer states on CPU
    #     _load_optimizer()
    #     self.logger.info("Optimizer resume complete")

    #     # -----------------------------
    #     # 2) Parse the resume epoch/iter
    #     # -----------------------------
    #     resume_epoch, resume_iteration = util.ckpt.split_ckpt_str_into_epoch_iter(
    ...
    #     # -----------------------------
    #     # 3) Load any additional rank-specific CPU data
    #     # -----------------------------
    #     additional_rank_specific = os.path.join(
    #         resume_path, f"additional.{dist.get_rank():05d}-of-{dist.get_world_size():05d}.pth"
    #     )
    ...
    #     # -----------------------------
    #     # 4) (Optional) Clear any existing GPU caches
    #     # -----------------------------
    #     # If your model was created on CPU, or not yet on GPU, this will just clear leftover allocations
    #     # from prior runs. If the model or checkpoint was already partially on GPU, calling this might help
    #     # remove fragmentation. It will NOT fix fundamental OOM if your total memory is still too large.
    #     torch.cuda.empty_cache()

    #     # -----------------------------
    #     # 5) Move the model from CPU to GPU
    #     # -----------------------------
    #     # If your code previously constructed self.model on CPU, do:
    #     # (If your code had already placed the model on GPU at init, you'll need to refactor that.)
    #     device = torch.device(f"cuda:{dist.get_rank()}")
    #     self.model.to(device)

    #     # -----------------------------
    #     # 6) Move the optimizer’s state tensors to GPU
    #     # -----------------------------
    #     # PyTorch doesn't automatically move optimizer states when you do .to(device) on the model.
    #     # You must manually move them. A common approach:
    #     for param in self.optimizer.state.keys():
    #         # param is a torch.Tensor (the parameter)
    #         # state is a dict of momentum, etc.
    ...
    #     # Now your model and optimizer states are fully on the GPU. 
    #     # Continue with your training loop as usual.
    ...
    def train_one_epoch(
        ...
        for data_iter_step, batch_data in enumerate(
            ...
            # import ipdb
            # ipdb.set_trace()
            # if "predict_next_frame" in self.pretrain_task:
            #     '''
            ...
            #     '''
            #     # if self.video_frames/self.video_fps > self.data_video_duration:
            #     #     raise ValueError(f"We do not have videos longer than ")
            #     #     print(self.video_frames/self.video_fps)
                
            #     # if video_max_frames

            #     fps_duration = random.choice(self.all_fps_duration)
            #     tokenized_fps_duration = self.tokenizer.encode(fps_duration, bos=False, eos=False)

            #     # 需要加入-100的mask

            #     # 你好
            #     # 我们先确认使用我们使用的frames数目不能超过长度，其次，秒数也不能超过长度。
